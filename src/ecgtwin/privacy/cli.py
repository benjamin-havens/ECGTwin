...
from .generation import (
    build_privacy_generation_tasks,
    generate_synthetic_latents,
    load_privacy_runtime,
    save_synthetic_pool,
    write_privacy_generation_task,
)
...
def _load_or_generate_synthetic_payload(
    ...
    """Load an existing synthetic pool generated by the shared Lightning executor."""
    payload_path = synthetic_root / task["subset"] / f"{task['cache_key']}.pt"
    if payload_path.exists():
        return load_dataset_file(str(payload_path))
    raise FileNotFoundError(f"Missing synthetic pool for task {task['cache_key']} at {payload_path}")
        ...
        for chunk in worker_batch["chunks"]:
            for subject_task in chunk["subjects"]:
                ...
                for local_index, record_index in enumerate(indices):
                    ...
                    ibe_scores = ibe_white_box_score(record, partner, ibe_model=conditioner_model, device=device)
                    rows_buffer.append(
                        {
...
def _run_lightning_synthetic_generation(cfg, subject_tasks: list[dict], synthetic_root: Path, output_root: Path) -> dict:
    """Generate all synthetic pools through the shared Lightning execution layer."""
    synthetic_root.mkdir(parents=True, exist_ok=True)
    tasks = build_privacy_generation_tasks(subject_tasks, synthetic_root)
    summary = run_generation_tasks(
        cfg,
        tasks,
        scope="privacy",
        output_root=output_root,
        task_handler=write_privacy_generation_task,
    )
    return {"task_count": len(tasks), "synthetic_root": str(synthetic_root), **summary}
...
def run_splits(config_path, overrides):
    ...
    return {"output_dir": str(output_dir), "manifest_path": str(output_dir / "audit_manifest.json")}
...
def run_generate(config_path, overrides):
    ...
    generation_summary = _run_lightning_synthetic_generation(cfg, tasks, synthetic_root, output_dir)
    logger.info(
        "Generated or refreshed %s synthetic pools across devices %s",
        generation_summary["task_count"],
        ", ".join(generation_summary["devices"]),
    )
    _persist_config(output_dir, cfg)
    return {"output_dir": str(output_dir), "manifest_path": str(output_dir / "audit_manifest.json"), "synthetic_root": str(synthetic_root)}
...
def run_audit(config_path, overrides):
    ...
    generation_summary = _run_lightning_synthetic_generation(cfg, tasks, synthetic_root, output_dir)
    logger.info(
        "Prepared %s synthetic pools across devices %s before privacy scoring",
        generation_summary["task_count"],
        ", ".join(generation_summary["devices"]),
    )
    ...
    return {
        ...
        "manifest_path": str(output_dir / "audit_manifest.json"),
        "metrics_path": str(output_dir / "metrics.json"),
        "scores_path": str(output_dir / "scores.csv"),
