"""ECG/text representation model and supporting 1D CNN backbone modules."""
...
class CLIP(nn.Module):
    ...
    def encode_signal(self, signal):
        ...
        # return (B, 1024) no matter what the input dim is
        return self.resnet(signal.type(self.dtype))
  
    def forward(self, signal, text_embed):
        # input signal: (B, L, C), text_embedding: (B, Dim)
        # signal embedding: (B, 1024)
        signal_embedding = self.encode_signal(signal)

        # signal & text features: (B, Dim)
        signal_features = self.ecg_projector(signal_embedding)
        text_features = self.text_projector(text_embed)
