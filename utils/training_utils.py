...
def train_epoch_channels(dataloader, 
    ...
    for _ in range(number_of_repetition):
        for ecg_ref, ecg_tar in dataloader:
            ...
            noise = torch.randn(latent_tar.shape, device=device)
            ...
            if use_recons_loss:
                # Compute alpha_t from scheduler
                alpha_t = diffused_model.alphas_cumprod.to(xt.device)[t].view(-1, 1, 1)  # shape matches (B, 1, 1)
                # Predict the original latent (x0)
                latent_pred = (xt - (1 - alpha_t).sqrt() * noise_estim) / alpha_t.sqrt()  # shape: (B, C, L)
                # Batchwise MSE loss between predicted x0 and ground truth
                loss_recons = F.mse_loss(latent_pred, latent_tar, reduction='sum').div(latent_tar.size(0))
...
def train_diffusion_model(meta, 
                        save_weights_path, 
                        dataloader,  
                        diffused_model, 
                        ibe_model,
